...
export const purchaseRequestService = {
  ...
  /**
   * Update purchase request (only DRAFT can be edited)
   * @param {string} id - Request ID
  ...
  async submitForApproval(id, user) {
    ...
    // await telegramService.notifyPurchaseRequestSubmitted(result.data);
  ...
  async approve(id, user) {
    ...
    if (!canManagePurchaseRequests(user.professional)) {
      return {
        ...
        error: {
          ...
          message: 'Apenas gerentes e administradores podem aprovar solicitações',
        },
      };
    }
    ...
    // TODO: Send Telegram notification to requester
    // await telegramService.notifyPurchaseRequestApproved(result.data);
  ...
  async reject(id, reason, user) {
    ...
    if (!canManagePurchaseRequests(user.professional)) {
      return {
        ...
        error: {
          type: 'PERMISSION_DENIED',
          message: 'Apenas gerentes e administradores podem rejeitar solicitações',
        },
      };
    }
    ...
    // TODO: Send Telegram notification to requester
    // await telegramService.notifyPurchaseRequestRejected(result.data);
  ...
  async getQuotesByRequest(requestId) {
    const result = await purchaseRequestRepository.getQuotesByRequest(requestId);
  ...
  async selectQuote(quoteId, reason, user) {
    ...
    if (!canManagePurchaseRequests(user.professional)) {
      return {
        ...
        error: {
          type: 'PERMISSION_DENIED',
          message: 'Apenas gerentes e administradores podem selecionar cotações',
        },
      };
    }
