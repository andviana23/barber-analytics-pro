...
const EditBankAccountModal = ({
  ...
  const validateForm = async () => {
    ...
    if (!newErrors.bank && !newErrors.agency && !newErrors.account_number && !newErrors.unit_id && account) {
      try {
        setChecking(true);
        const exists = await checkAccountExists(formData.bank.trim(), formData.agency.trim(), formData.account_number.trim(), formData.unit_id, account.id // Excluir a conta atual da verificação
        );
        if (exists) {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      showSuccess('Conta bancária atualizada', `A conta ${formData.name} foi atualizada com sucesso.`);
      setErrors({});
      setTouched({});
        ...
        <div className="card-theme relative w-full max-w-lg transform rounded-lg shadow-xl transition-all dark:bg-dark-surface">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 p-6">
            ...
            <div className="flex items-center gap-3 rounded-lg bg-light-bg p-4 dark:bg-dark-bg dark:bg-gray-700">
              <input type="checkbox" name="is_active" id="is_active" checked={formData.is_active} onChange={handleInputChange} disabled={loading} className="h-4 w-4 rounded border-light-border text-blue-600 focus:ring-blue-500 dark:border-dark-border" />
              <label htmlFor="is_active" className="text-theme-primary dark:text-dark-text-primary text-sm font-medium">
                Conta ativa
              </label>
