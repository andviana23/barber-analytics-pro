...
const OrderHistoryPage = () => {
  ...
  return <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button onClick={exportToCSV} loading={exporting} disabled={filteredOrders.length === 0} icon={<Download className="h-4 w-4" />}>
          Exportar CSV
        </Button>
      </div>
      ...
      <Card className="p-4">
        <div className="mb-4 flex items-center justify-between">
          ...
          <Button variant="secondary" size="sm" onClick={clearFilters} icon={<X className="h-4 w-4" />}>
            Limpar Filtros
          </Button>
        </div>
      ...
      {loading ? <div className="py-12 text-center">
          ...
          <div className="space-y-3">
            {paginatedOrders.map(order => <Card key={order.id} className="p-4 transition-shadow hover:shadow-md">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="mb-2 flex items-center gap-3">
                      ...
                      {order.discount_value > 0 && <span className="rounded bg-green-100 px-2 py-1 text-xs text-green-700">
                          Desconto
                        </span>}
                      {order.fee_value > 0 && <span className="rounded bg-orange-100 px-2 py-1 text-xs text-orange-700">
                          Taxa
                        </span>}
                    </div>
                    ...
                    {order.items && order.items.length > 0 && <div className="mt-3 border-t border-light-border pt-3 dark:border-dark-border">
                        <p className="text-theme-secondary mb-2 text-xs">
                          Itens:
                        </p>
                        <div className="flex flex-wrap gap-2">
                          {order.items.map(item => <span key={item.id} className="card-theme rounded px-2 py-1 text-xs">
