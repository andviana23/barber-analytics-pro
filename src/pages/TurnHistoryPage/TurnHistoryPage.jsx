...
const TurnHistoryPage = () => {
  ...
  const handleExportCSV = () => {
    if (tableData.length === 0) {
      showToast({ type: 'warning', message: 'Não há dados para exportar' });
      return;
    }
    ...
    showToast({ type: 'success', message: 'CSV exportado com sucesso!' });
  };
              ...
              <thead className="text-xs uppercase bg-gray-100 dark:bg-gray-800">
                <tr>
                  ...
                  {professionals.map(prof => (
                    <th
                      key={prof}
                      className="px-4 py-3 text-center text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800"
                    >
                      {prof}
                    </th>
                  ))}
                  <th className="px-4 py-3 text-center font-bold text-gray-900 dark:text-white bg-gray-200 dark:bg-gray-700">
                    Total
                  </th>
                </tr>
              </thead>
              <tbody>
                ...
                <tr className="bg-gray-200 dark:bg-gray-700 font-bold">
                  ...
                  {professionals.map(prof => (
                    <td
                      key={prof}
                      className="px-4 py-3 text-center text-gray-900 dark:text-white"
                    >
                      {totals[prof] || 0}
                    </td>
                  ))}
                  <td className="px-4 py-3 text-center text-gray-900 dark:text-white bg-gray-300 dark:bg-gray-600">
                    {totals.total || 0}
                  </td>
                </tr>
              </tbody>
