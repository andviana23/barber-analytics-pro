...
import { Activity, BarChart3, Calendar, ChevronLeft, DollarSign, Download, Eye, FileText, Filter, PieChart, Printer, RefreshCw, Settings, Share2, Target, TrendingUp, Users } from 'lucide-react';
...
const FiltrosAvancados = ({
  ...
  return <div className="card-theme rounded-lg border border-light-border p-4 dark:border-dark-border dark:bg-dark-surface">
      <div className="mb-4 flex items-center justify-between">
        ...
        <Button variant="ghost" size="sm" onClick={() => setIsExpanded(!isExpanded)} className="text-theme-secondary dark:text-light-text-muted dark:text-dark-text-muted hover:text-gray-700 dark:text-gray-300 dark:text-gray-600 dark:text-theme-secondary dark:hover:text-gray-200">
      ...
      {isExpanded && <div className="mt-4 border-t border-light-border pt-4 dark:border-dark-border">
      ...
      <div className="mt-4 border-t border-light-border pt-4 dark:border-dark-border">
        <Button onClick={onApplyFilters} className="text-dark-text-primary flex w-full items-center justify-center gap-2 rounded-lg bg-blue-600 py-2.5 font-medium transition-colors hover:bg-blue-700">
...
const RelatorioCard = ({
  ...
  return <div onClick={onClick} className={`relative cursor-pointer rounded-xl border-2 p-6 transition-all duration-200 ${isActive ? 'border-blue-500 bg-blue-50 shadow-lg dark:bg-blue-900/20' : 'border-gray-200 bg-white hover:border-gray-300 hover:shadow-md dark:border-gray-700 dark:bg-gray-800 dark:hover:border-gray-600'}`}>
      {loading && <div className="card-theme/80 absolute inset-0 flex items-center justify-center rounded-xl dark:bg-dark-surface/80">
          <RefreshCw className="h-6 w-6 animate-spin text-blue-500" />
        </div>}

      <div className="flex items-start space-x-4">
        <div className={`rounded-lg p-3 ${report.color} flex-shrink-0 text-white`}>
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className={`mt-1 text-sm ${isActive ? 'text-blue-700 dark:text-blue-300' : 'text-gray-600 dark:text-gray-400'}`}>
            {report.description}
          </p>
          <div className="mt-3 flex items-center space-x-2">
            <span className={`rounded-full px-2 py-1 text-xs font-medium ${report.category === 'financeiro' ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400' : report.category === 'operacional' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400' : 'bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-purple-400'}`}>
              {report.category}
            </span>
            {report.featured && <span className="rounded-full bg-yellow-100 px-2 py-1 text-xs font-medium text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400">
                Destacado
              </span>}
          </div>
        </div>
...
const MetricasRapidas = ({
  ...
  if (loading) {
    return <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        {[...Array(4)].map((_, i) => <div key={i} className="card-theme rounded-lg border border-light-border p-4 dark:border-dark-border dark:bg-dark-surface">
  ...
  return <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
      ...
      return <div key={index} className={`${metric.bgColor} rounded-lg border border-gray-200 p-4 dark:border-gray-700`}>
    ...
    return <div className="space-y-6">
        ...
        <div className="card-theme rounded-lg border border-light-border dark:border-dark-border dark:bg-dark-surface">
          {loading ? <div className="flex items-center justify-center py-12">
            ...
            </div> : activeReport === 'dre-mensal' ? <div className="p-6">
              <DREDynamicView dreData={dreData} isLoading={loading} />
            </div> : activeReport === 'fluxo-caixa' ? <RelatorioFluxoCaixa filters={filters} units={allUnits || []} /> : activeReport === 'comparativo-unidades' ? <RelatorioComparativoUnidades filters={{
