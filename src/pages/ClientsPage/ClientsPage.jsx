  ...
  return <Layout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          {canManage && <button onClick={() => setIsCreateModalOpen(true)} className="text-dark-text-primary flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 transition-colors hover:bg-blue-700">
        ...
        <div className="card-theme rounded-lg border border-light-border p-4 shadow-sm dark:border-dark-border dark:bg-dark-surface">
          <div className="flex flex-col gap-4 md:flex-row">
            ...
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Buscar por nome, CPF, telefone ou email..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="card-theme text-theme-primary dark:text-dark-text-primary w-full rounded-lg border border-light-border py-2 pl-10 pr-4 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 dark:border-dark-border dark:bg-gray-700" />
            </div>

            {/* Filtro de inativos */}
            <div className="flex items-center">
              <label className="flex cursor-pointer items-center">
                <input type="checkbox" checked={showInactive} onChange={e => setShowInactive(e.target.checked)} className="h-4 w-4 rounded border-light-border text-blue-600 focus:ring-blue-500 dark:border-dark-border" />
        ...
        <div className="card-theme overflow-hidden rounded-lg border border-light-border shadow-sm dark:border-dark-border dark:bg-dark-surface">
          {loading ? <div className="flex h-96 items-center justify-center">
            ...
            </div> : error ? <div className="flex h-96 items-center justify-center">
              <div className="text-center">
            ...
            </div> : filteredClients.length === 0 ? <div className="flex h-96 items-center justify-center">
              <div className="text-center">
                <Users className="text-light-text-muted dark:text-dark-text-muted mx-auto mb-4 h-16 w-16" />
                <h3 className="text-theme-primary dark:text-dark-text-primary mb-2 text-lg font-medium">
                ...
                <table className="w-full">
                  <thead className="bg-light-bg dark:bg-dark-bg dark:bg-gray-700">
                    <tr>
                      ...
                      {canManage && <th className="text-theme-secondary dark:text-light-text-muted dark:text-dark-text-muted px-6 py-3 text-right text-xs font-medium uppercase tracking-wider">
                          Ações
                        </th>}
                    </tr>
                  </thead>
                  <tbody className="card-theme divide-y divide-gray-200 dark:divide-gray-700 dark:bg-dark-surface">
                    {paginatedClients.map(client => <tr key={client.id} className="transition-colors hover:bg-light-bg dark:bg-dark-bg dark:hover:bg-gray-700">
                        ...
                        <td className="px-6 py-4">
                          <div className="space-y-1">
                            {client.telefone && <div className="text-theme-secondary dark:text-light-text-muted dark:text-dark-text-muted flex items-center text-sm">
                                <Phone className="mr-2 h-4 w-4" />
                                {formatPhone(client.telefone)}
                              </div>}
                            {client.email && <div className="text-theme-secondary dark:text-light-text-muted dark:text-dark-text-muted flex items-center text-sm">
                                <Mail className="mr-2 h-4 w-4" />
                                {client.email}
                              </div>}
                            {!client.telefone && !client.email && <span className="text-light-text-muted dark:text-dark-text-muted text-sm">
                                -
                              </span>}
                          </div>
                        </td>
                        ...
                        <td className="whitespace-nowrap px-6 py-4">
                          {client.is_active ? <span className="inline-flex items-center rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-800 dark:bg-green-900 dark:text-green-200">
                              <CheckCircle className="mr-1 h-3 w-3" />
                              Ativo
                            </span> : <span className="inline-flex items-center rounded-full bg-red-100 px-2.5 py-0.5 text-xs font-medium text-red-800 dark:bg-red-900 dark:text-red-200">
                        ...
                        setIsEditModalOpen(true);
                              ...
                              {client.is_active ? <button onClick={() => handleDeleteClient(client.id)} disabled={deletingId === client.id} className="text-red-600 hover:text-red-900 disabled:opacity-50 dark:text-red-400 dark:hover:text-red-300" title="Desativar">
                                  {deletingId === client.id ? <Loader className="h-5 w-5 animate-spin" /> : <Trash2 className="h-5 w-5" />}
              ...
              {/* Paginação */}
              {totalPages > 1 && <div className="flex items-center justify-between border-t border-light-border px-6 py-4 dark:border-dark-border">
                  <div className="text-sm text-gray-700 dark:text-gray-300 dark:text-gray-600 dark:text-theme-secondary">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} disabled={currentPage === 1} className="rounded-lg border border-light-border px-3 py-1 hover:bg-light-bg disabled:cursor-not-allowed disabled:opacity-50 dark:border-dark-border dark:bg-dark-bg dark:hover:bg-gray-700">
                      Anterior
                    </button>
                    ...
                    <div className="flex items-center gap-1">
                      {Array.from({
                  length: totalPages
                }, (_, i) => i + 1).map(page => <button key={page} onClick={() => setCurrentPage(page)} className={`rounded-lg px-3 py-1 ${currentPage === page ? 'bg-blue-600 text-white' : 'border border-gray-300 hover:bg-gray-50 dark:border-gray-600 dark:hover:bg-gray-700'}`}>
                          {page}
                        </button>)}
                    </div>

                    <button onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} disabled={currentPage === totalPages} className="rounded-lg border border-light-border px-3 py-1 hover:bg-light-bg disabled:cursor-not-allowed disabled:opacity-50 dark:border-dark-border dark:bg-dark-bg dark:hover:bg-gray-700">
                      Próxima
                    </button>
                  </div>
                </div>}
