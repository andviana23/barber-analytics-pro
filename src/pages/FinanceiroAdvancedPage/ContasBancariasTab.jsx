...
import { Plus, Search, Building2, CreditCard, TrendingUp, TrendingDown, DollarSign, Calendar, MapPin, Phone, Mail, Edit, Edit3, Trash2, Eye, EyeOff, RefreshCw, Filter, Download, Upload, CheckCircle, XCircle, AlertCircle, Activity, BarChart3, PieChart, Users, Settings, MoreVertical, ChevronRight, Star, Shield, Zap } from 'lucide-react';
...
const ContasBancariasTab = ({
  ...
  console.log('🏦 ContasBancariasTab - Contas carregadas:', bankAccounts?.length || 0, bankAccounts);
  ...
  const filteredAndSortedAccounts = useMemo(() => {
    ...
    let filtered = bankAccounts.filter(account => {
      // Filtro de busca
      const matchesSearch = searchTerm === '' || account.name?.toLowerCase().includes(searchTerm.toLowerCase()) || account.bank?.toLowerCase().includes(searchTerm.toLowerCase()) || account.agency?.toLowerCase().includes(searchTerm.toLowerCase()) || account.account_number?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const stats = useMemo(() => {
    ...
    const totalInitialBalance = bankAccounts.reduce((sum, acc) => sum + (parseFloat(acc.initial_balance) || 0), 0);
    const totalCurrentBalance = bankAccounts.reduce((sum, acc) => sum + (parseFloat(acc.current_balance) || 0), 0);
    const totalAvailableBalance = bankAccounts.reduce((sum, acc) => sum + (parseFloat(acc.available_balance) || 0), 0);
    const totalRevenues = bankAccounts.reduce((sum, acc) => sum + (parseFloat(acc.total_revenues) || 0), 0);
    const totalExpenses = bankAccounts.reduce((sum, acc) => sum + (parseFloat(acc.total_expenses) || 0), 0);
    ...
    const topBank = Object.keys(bankCounts).reduce((a, b) => bankCounts[a] > bankCounts[b] ? a : b, '');
    return {
      total,
      active,
      inactive,
  ...
  const AccountCard = ({
    ...
    const balanceVariation = (account.current_balance || 0) - (account.initial_balance || 0);
    const isPositiveVariation = balanceVariation >= 0;
    return <div className="group card-theme dark:bg-dark-surface rounded-2xl border border-light-border dark:border-dark-border p-6 hover:shadow-2xl hover:border-blue-400 dark:hover:border-blue-500 transition-all duration-300 hover:-translate-y-1">
        {/* Header do Card */}
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center space-x-4">
            <div className="relative">
              ...
              {account.is_active && <div className="absolute -top-1 -right-1 w-4 h-4 bg-green-500 rounded-full border-2 border-light-surface dark:border-dark-surface dark:border-gray-800 animate-pulse" />}
            </div>
            <div>
          ...
          {canManage && <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button onClick={() => handleEdit(account)} className="p-2 text-light-text-muted dark:text-dark-text-muted hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-all" title="Editar conta">
                <Edit className="w-4 h-4" />
              </button>
              <button onClick={() => handleDelete(account)} className="p-2 text-light-text-muted dark:text-dark-text-muted hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-all" title="Excluir conta">
        ...
        <div className="space-y-3 mb-5">
          ...
          <div className="p-4 bg-gradient-to-br from-blue-50 to-cyan-50 dark:from-blue-900/20 dark:to-cyan-900/20 rounded-xl border border-blue-100 dark:border-blue-800/30">
            <div className="flex items-center justify-between">
              ...
              {canManage && <button onClick={() => handleEditBalance(account)} className="p-2 text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-900/40 rounded-lg transition-all" title="Editar saldo inicial">
          ...
          <div className="p-4 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-xl border border-green-100 dark:border-green-800/30">
            <div className="flex items-center justify-between">
              ...
              {balanceVariation !== 0 && <div className={`flex items-center space-x-1 px-2 py-1 rounded-lg ${isPositiveVariation ? 'bg-green-100 dark:bg-green-900/30' : 'bg-red-100 dark:bg-red-900/30'}`}>
                  {isPositiveVariation ? <TrendingUp className="w-4 h-4 text-green-600 dark:text-green-400" /> : <TrendingDown className="w-4 h-4 text-red-600 dark:text-red-400" />}
                  <span className={`text-xs font-semibold ${isPositiveVariation ? 'text-green-700 dark:text-green-300' : 'text-red-700 dark:text-red-300'}`}>
        ...
        {(account.total_revenues > 0 || account.total_expenses > 0) && <div className="pt-4 border-t border-light-border dark:border-dark-border mb-4">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-light-border dark:border-dark-border">
          <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${account.is_active ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300'}`}>
            {account.is_active ? <>
  ...
  const TableRow = ({
    account
      ...
      <td className="px-6 py-4 whitespace-nowrap">
        <div className="flex items-center">
          <div className="relative">
            ...
            {account.is_active && <div className="absolute -top-1 -right-1 w-3 h-3 bg-green-500 rounded-full border border-light-surface dark:border-dark-surface"></div>}
          </div>
          <div className="ml-4">
      ...
      <td className="px-6 py-4 whitespace-nowrap">
        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${account.is_active ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300'}`}>
      ...
      {canManage && <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
          <div className="flex items-center justify-end space-x-2">
            <button onClick={() => handleEdit(account)} className="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300 transition-colors">
              <Edit className="w-4 h-4" />
            </button>
            <button onClick={() => handleDelete(account)} className="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 transition-colors">
              <Trash2 className="w-4 h-4" />
            </button>
          </div>
        </td>}
  ...
  if (loading) {
    return <div className="space-y-6">
        {/* Header com Loading */}
        <div className="bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 rounded-2xl p-8 text-dark-text-primary">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {Array.from({
          length: 4
        }).map((_, i) => <div key={i} className="card-theme dark:bg-dark-surface rounded-xl p-6 animate-pulse">
  ...
  if (error) {
    return <div className="flex flex-col items-center justify-center min-h-[400px] p-6">
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-8 text-center max-w-md">
          ...
          <button onClick={handleRefresh} className="inline-flex items-center px-4 py-2 bg-red-600 text-dark-text-primary rounded-lg hover:bg-red-700 transition-colors">
  ...
  return <div className="space-y-6">
      ...
      <div className="card-theme dark:bg-dark-surface rounded-lg border border-light-border dark:border-dark-border p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center space-x-3">
            <button onClick={handleRefresh} className="p-2 text-light-text-muted dark:text-dark-text-muted hover:text-theme-secondary dark:hover:text-gray-300 dark:text-gray-600 transition-colors" title="Atualizar">
              <RefreshCw className="w-5 h-5" />
            </button>
            {canManage && <button onClick={handleCreate} className="inline-flex items-center px-4 py-2 bg-blue-600 text-dark-text-primary text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-gradient-to-br from-gray-50 to-slate-50 dark:from-gray-700/50 dark:to-slate-700/50 rounded-xl p-5 border border-light-border dark:border-dark-border">
            <div className="flex items-center justify-between">
              <div>
        ...
        {/* Resumo de Movimentações */}
        {(stats.totalRevenues > 0 || stats.totalExpenses > 0) && <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
          <div className="flex flex-col sm:flex-row gap-3 flex-1">
            ...
            <div className="relative">
              ...
              <input type="text" placeholder="Buscar por nome, banco, agência ou conta..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 w-full sm:w-80 px-4 py-2 border border-light-border dark:border-dark-border rounded-lg card-theme dark:bg-gray-700 text-theme-primary dark:text-dark-text-primary placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200" />
            </div>

            {/* Filtro por banco */}
            <select value={selectedBank} onChange={e => setSelectedBank(e.target.value)} className="px-4 py-2 border border-light-border dark:border-dark-border rounded-lg card-theme dark:bg-gray-700 text-theme-primary dark:text-dark-text-primary focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
              <option value="all">Todos os Bancos</option>
              {uniqueBanks.map(bank => <option key={bank} value={bank}>
                  {bank}
                </option>)}
            </select>
          ...
          <div className="flex items-center gap-3">
            ...
            <label className="flex items-center space-x-2 cursor-pointer">
              <input type="checkbox" checked={showInactive} onChange={e => setShowInactive(e.target.checked)} className="rounded border-light-border dark:border-dark-border text-blue-600 focus:ring-blue-500" />
            ...
            <div className="flex card-theme dark:bg-gray-700 rounded-lg p-1">
              <button onClick={() => setViewMode('grid')} className={`p-2 rounded-md transition-colors ${viewMode === 'grid' ? 'bg-white dark:bg-gray-600 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}`} title="Visualização em Grid">
                <BarChart3 className="w-5 h-5" />
              </button>
              <button onClick={() => setViewMode('list')} className={`p-2 rounded-md transition-colors ${viewMode === 'list' ? 'bg-white dark:bg-gray-600 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}`} title="Visualização em Lista">
      ...
      <div className="card-theme dark:bg-dark-surface rounded-xl border border-light-border dark:border-dark-border overflow-hidden">
        {filteredAndSortedAccounts.length === 0 ? <div className="p-12 text-center">
            ...
            <p className="text-theme-secondary dark:text-light-text-muted dark:text-dark-text-muted mb-6">
              {bankAccounts?.length === 0 ? 'Cadastre a primeira conta bancária para começar.' : 'Tente ajustar os filtros de busca.'}
            </p>
            {bankAccounts?.length === 0 && canManage && <button onClick={handleCreate} className="inline-flex items-center px-6 py-3 bg-blue-600 text-dark-text-primary font-semibold rounded-lg hover:bg-blue-700 transition-colors">
                <Plus className="w-5 h-5 mr-2" />
                Cadastrar Primeira Conta
              </button>}
          </div> : viewMode === 'grid' ? <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredAndSortedAccounts.map(account => <AccountCard key={account.id} account={account} />)}
            </div>
            ...
            <table className="w-full">
              ...
              <tbody className="card-theme dark:bg-dark-surface divide-y divide-gray-200 dark:divide-gray-700">
                {filteredAndSortedAccounts.map(account => <TableRow key={account.id} account={account} />)}
              </tbody>
            </table>
          </div>}
      </div>
      ...
      {/* Aviso de Permissões */}
      {!canManage && <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl p-4">
      ...
      {/* Modals */}
      <CreateBankAccountModal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} onSuccess={handleSuccess} />

      <EditBankAccountModal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} onSuccess={handleSuccess} account={selectedAccount} />

      <DeleteBankAccountModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onSuccess={handleSuccess} account={selectedAccount} />

      <EditInitialBalanceModal isOpen={isEditBalanceModalOpen} onClose={() => setIsEditBalanceModalOpen(false)} onSuccess={handleSuccess} account={selectedAccount} />
    </div>;
};
