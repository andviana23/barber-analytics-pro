...
import { CreditCard, Plus, Search, ChevronDown, Calendar, DollarSign, Building2, Edit, Eye, ArrowDown, X, CheckCircle, Clock, AlertCircle, Trash2 } from 'lucide-react';
...
const DespesasAccrualTab = ({
  ...
  const fetchBankAccounts = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from('bank_accounts').select('id, name, bank_name, account_number').eq('unit_id', globalFilters.unitId).eq('is_active', true).order('name');
  ...
  const fetchExpenses = async () => {
    ...
    try {
      ...
      if (!hasManualFilters) {
        ...
        query = query.gte('expected_payment_date', startOfMonth.toISOString().split('T')[0]).lte('expected_payment_date', endOfMonth.toISOString().split('T')[0]);
  ...
  const filteredExpenses = useMemo(() => {
    ...
    return expenses.filter(expense => expense.description?.toLowerCase().includes(searchTerm.toLowerCase()) || expense.party?.nome?.toLowerCase().includes(searchTerm.toLowerCase()) || expense.category?.name?.toLowerCase().includes(searchTerm.toLowerCase()));
  }, [expenses, searchTerm]);

  // Calcular totais
  const totals = useMemo(() => {
    const totalTitles = filteredExpenses.reduce((sum, expense) => sum + (expense.value || 0), 0);
    const totalPaid = filteredExpenses.reduce((sum, expense) => sum + (expense.status === 'Paid' ? expense.value || 0 : 0), 0);
  ...
  return <div className="flex h-full w-full flex-col overflow-hidden">
      ...
      <div className="card-theme mb-2 rounded-lg border border-light-border p-2 shadow-sm dark:border-dark-border dark:bg-dark-surface">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex min-w-[300px] flex-1 items-center gap-2">
            ...
            <input type="date" value={filters.dueDateFrom} onChange={e => setFilters(prev => ({
            ...prev,
            ...
            <input type="date" value={filters.dueDateTo} onChange={e => setFilters(prev => ({
            ...prev,
            dueDateTo: e.target.value
          }))} className="card-theme text-theme-primary dark:text-dark-text-primary flex-1 rounded border border-light-border px-2 py-1 text-xs dark:border-dark-border dark:bg-gray-700" />
          </div>
          <button onClick={fetchExpenses} className="text-dark-text-primary flex items-center rounded bg-green-600 px-2 py-1 text-xs transition-colors hover:bg-green-700 dark:bg-green-700 dark:hover:bg-green-800">
            <Search className="mr-1 h-3 w-3" />
            Buscar
          </button>
          <button onClick={clearFilters} className="btn-theme-secondary flex items-center rounded px-2 py-1 text-xs transition-colors">
      ...
      <div className="card-theme flex min-h-0 flex-1 flex-col rounded-lg border border-light-border shadow-sm dark:border-dark-border dark:bg-dark-surface">
        ...
        <div className="flex flex-shrink-0 flex-wrap items-center justify-between gap-2 border-b border-light-border p-2 dark:border-dark-border">
          <div className="flex items-center gap-2">
            ...
            <ImportExpensesFromOFXButton onImportSuccess={result => {
            showToast({
              type: 'success',
          ...
          <div className="flex items-center gap-1">
            ...
            <input type="text" placeholder="Pesquisar" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="card-theme text-theme-primary dark:text-dark-text-primary w-48 rounded border border-light-border px-2 py-1 text-xs dark:border-dark-border dark:bg-gray-700" />
          </div>
        </div>
          ...
          <table className="w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody className="card-theme divide-y divide-gray-200 dark:divide-gray-700 dark:bg-dark-surface">
              {loading ? <tr>
                  ...
                  <td colSpan="11" className="text-theme-secondary dark:text-light-text-muted dark:text-dark-text-muted px-4 py-8 text-center">
                    Nenhuma despesa encontrada
                  </td>
                </tr> : filteredExpenses.map(expense => <tr key={expense.id} className="transition-colors hover:bg-light-bg dark:bg-dark-bg dark:hover:bg-gray-700">
                    ...
                    <td className="text-theme-secondary dark:text-light-text-muted dark:text-dark-text-muted whitespace-nowrap px-2 py-1 text-[11px]">
                      {expense.date ? format(parseISO(expense.date), 'dd/MM/yy', {
                  locale: ptBR
                }) : '-'}
                    </td>
                    ...
                    <td className="text-theme-secondary dark:text-light-text-muted dark:text-dark-text-muted whitespace-nowrap px-2 py-1 text-[11px]">
                      {expense.expected_payment_date ? format(parseISO(expense.expected_payment_date), 'dd/MM/yy', {
                  locale: ptBR
                }) : '-'}
                    </td>
                  ...
                  currency: 'BRL'
                    ...
                    <td className="text-theme-secondary dark:text-light-text-muted dark:text-dark-text-muted whitespace-nowrap px-2 py-1 text-right text-[11px]">
                      {expense.status === 'Paid' ? expense.value?.toLocaleString('pt-BR', {
                  style: 'currency',
                  currency: 'BRL'
                }) : 'R$ 0,00'}
                    </td>
                    <td className="text-theme-secondary dark:text-light-text-muted dark:text-dark-text-muted whitespace-nowrap px-2 py-1 text-[11px]">
                      {expense.actual_payment_date ? format(parseISO(expense.actual_payment_date), 'dd/MM/yy', {
                  locale: ptBR
                }) : '-'}
                    </td>
                    <td className="px-2 py-1 text-center">
                      <span className={`inline-flex whitespace-nowrap rounded-full px-1.5 py-0.5 text-[10px] font-semibold ${getStatusColor(expense.status)}`}>
                        {getStatusText(expense.status)}
                      </span>
                    </td>
                    <td className="px-2 py-1 text-center">
                      <div className="dropdown-container relative">
                        <button onClick={() => setExpenseActionsOpen(expenseActionsOpen === expense.id ? null : expense.id)} className="text-light-text-muted dark:text-dark-text-muted hover:text-theme-secondary hover:card-theme flex h-8 w-8 items-center justify-center rounded-full transition-colors dark:hover:bg-gray-700">
                          <ChevronDown className="h-4 w-4" />
                        </button>

                        {expenseActionsOpen === expense.id && <div className="card-theme absolute bottom-full right-0 z-[9999] mb-1 w-48 overflow-hidden rounded-lg border border-light-border shadow-2xl dark:border-dark-border dark:bg-dark-surface">
                            <div className="py-1">
                              <button onClick={() => {
                        ...
                        setExpenseActionsOpen(null);
                              ...
                              {expense.status !== 'Paid' && <button onClick={() => {
        ...
        <div className="flex flex-shrink-0 items-center justify-between border-t border-light-border px-4 py-3 dark:border-dark-border">
          ...
          <div className="flex items-center space-x-2">
            ...
            <button className="text-theme-secondary hover:text-theme-secondary px-3 py-1 text-sm dark:hover:text-gray-300 dark:text-gray-600">
              3
            </button>
            <button className="text-theme-secondary hover:text-theme-secondary px-3 py-1 text-sm dark:hover:text-gray-300 dark:text-gray-600">
              Próximo
      ...
      {/* Modais */}
      <ExpenseDetailsModal expense={selectedExpenseForAction} isOpen={isDetailsModalOpen} onClose={() => setIsDetailsModalOpen(false)} />

      <ExpenseEditModal expense={selectedExpenseForAction} isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} onSave={fetchExpenses} />

      <DeleteConfirmationModal expense={selectedExpenseForAction} isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onDelete={fetchExpenses} />

      {/* Modal de Dados da Baixa - Versão Melhorada */}
      {isPaymentModalOpen && selectedExpenseForPayment && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="card-theme mx-4 max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl shadow-2xl dark:bg-dark-surface">
            ...
            <div className="flex items-center justify-between border-b border-light-border p-6 dark:border-dark-border">
              ...
              <button onClick={() => setIsPaymentModalOpen(false)} className="text-light-text-muted dark:text-dark-text-muted hover:text-theme-secondary transition-colors">
                <X className="h-6 w-6" />
              </button>
            </div>
            ...
            <div className="space-y-6 p-6">
              ...
              <div>
                ...
                <input type="date" value={paymentData.paymentDate} onChange={e => setPaymentData(prev => ({
              ...prev,
              ...
              <div>
                ...
                {selectedExpenseForPayment?.account_id ? <div className="card-theme text-theme-secondary w-full rounded-lg border border-light-border px-4 py-3 dark:border-dark-border dark:bg-dark-hover">
                    <div className="flex items-center justify-between">
                      <span>
                        {selectedExpenseForPayment.account?.name} -{' '}
                        {selectedExpenseForPayment.account?.bank_name}
                        {selectedExpenseForPayment.account?.account_number && ` (${selectedExpenseForPayment.account.account_number})`}
                      </span>
                      <span className="text-theme-secondary rounded bg-light-hover px-2 py-1 text-xs dark:bg-dark-surface">
                        Banco Original
                      </span>
                    </div>
                  </div> : <select value={paymentData.bankId} onChange={e => setPaymentData(prev => ({
              ...prev,
              bankId: e.target.value
            }))} className="card-theme text-theme-primary dark:text-dark-text-primary w-full rounded-lg border border-light-border px-4 py-3 transition-colors focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-dark-border dark:bg-gray-700">
                    <option value="">Selecione o banco</option>
                    {bankAccounts.map(bank => <option key={bank.id} value={bank.id}>
                        {bank.name} - {bank.bank_name}
                        {bank.account_number && ` (${bank.account_number})`}
                      </option>)}
                  </select>}
                {bankAccounts.length === 0 && !selectedExpenseForPayment?.account_id && <p className="mt-1 text-xs text-amber-600 dark:text-amber-400">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={paymentData.paidValue} onChange={e => handleCurrencyChange('paidValue', e.target.value)} placeholder="0,00" className="card-theme text-theme-primary dark:text-dark-text-primary w-full rounded-lg border border-light-border py-3 pl-8 pr-4 transition-colors focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-dark-border dark:bg-gray-700" />
                  </div>
                </div>
                <div>
                  <label className="text-theme-secondary mb-2 block text-sm font-medium">
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={paymentData.interestValue} onChange={e => handleCurrencyChange('interestValue', e.target.value)} placeholder="0,00" className="card-theme text-theme-primary dark:text-dark-text-primary w-full rounded-lg border border-light-border py-3 pl-8 pr-4 transition-colors focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-dark-border dark:bg-gray-700" />
                  </div>
                </div>
                <div>
                  <label className="text-theme-secondary mb-2 block text-sm font-medium">
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={paymentData.discountValue} onChange={e => handleCurrencyChange('discountValue', e.target.value)} placeholder="0,00" className="card-theme text-theme-primary dark:text-dark-text-primary w-full rounded-lg border border-light-border py-3 pl-8 pr-4 transition-colors focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-dark-border dark:bg-gray-700" />
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea value={paymentData.observation} onChange={e => setPaymentData(prev => ({
              ...prev,
              observation: e.target.value
            }))} rows={3} placeholder="Informações adicionais sobre o pagamento..." className="card-theme text-theme-primary dark:text-dark-text-primary w-full resize-none rounded-lg border border-light-border px-4 py-3 transition-colors focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-dark-border dark:bg-gray-700" />
              </div>
            </div>
            ...
            <div className="rounded-b-xl border-t border-light-border bg-light-bg p-6 dark:border-dark-border dark:bg-gray-700/50">
              <div className="flex space-x-3">
                <button onClick={() => setIsPaymentModalOpen(false)} className="btn-theme-secondary flex-1 rounded-lg px-4 py-3 font-medium transition-colors">
                  Cancelar
                </button>
                <button onClick={processPayment} disabled={paymentLoading || !paymentData.bankId && !selectedExpenseForPayment?.account_id || !paymentData.paidValue} className="text-dark-text-primary flex flex-1 items-center justify-center rounded-lg bg-green-600 px-4 py-3 font-medium transition-colors hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-green-700 dark:hover:bg-green-800">
                  {paymentLoading ? <>
