 ...
 */
const DespesasAccrualTabRefactored = ({
  globalFilters
}) => {
  ...
  const filteredExpenses = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(e => e.description?.toLowerCase().includes(search) || e.party?.nome?.toLowerCase().includes(search) || e.category?.name?.toLowerCase().includes(search));
    }
    ...
    if (filters.status !== 'all') {
      ...
      if (filters.status === 'paid') {
        ...
        filtered = filtered.filter(e => e.status?.toLowerCase() === 'pending' && !isBefore(parseISO(e.expected_payment_date), now));
  ...
  const getStatusBadge = expense => {
    ...
    const isOverdue = isBefore(dueDate, now) && expense.status?.toLowerCase() !== 'paid';
    if (expense.status?.toLowerCase() === 'paid') {
      return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400">
          <CheckCircle className="w-3 h-3 mr-1" />
          Pago
        </span>;
    }
    if (isOverdue) {
      return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400">
          <AlertCircle className="w-3 h-3 mr-1" />
          Atrasado
        </span>;
    }
    return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400">
  ...
  return <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
            ...
            <div className="relative flex-1 max-w-md">
              ...
              <input type="text" placeholder="Buscar por descrição, fornecedor ou categoria..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-11 pr-4 py-3 card-theme dark:bg-dark-surface border-2 border-light-border dark:border-dark-border rounded-xl text-theme-primary placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 shadow-sm hover:shadow-md" />
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              setSelectedExpenseForAction(null);
              setIsEditModalOpen(true);
            }} className="flex items-center gap-2 px-5 py-3 bg-gradient-to-r bg-gradient-primary hover:from-blue-700 hover:to-indigo-700 text-dark-text-primary font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-105">
                <Plus className="w-5 h-5" />
                Nova Despesa
              </button>

              {/* Botão Importar OFX */}
              <ImportExpensesFromOFXButton unitId={globalFilters.unitId} onSuccess={fetchExpenses} />
            </div>
          </div>

          {/* Linha 2: Ações de Seleção Múltipla */}
          {filteredExpenses.length > 0 && <div className="flex items-center gap-3 pt-4 border-t-2 border-light-border dark:border-dark-border">
              <button onClick={() => {
            if (selectedExpenses.length === filteredExpenses.length) {
              ...
              setSelectedExpenses(filteredExpenses.map(e => e.id));
                ...
                {selectedExpenses.length === filteredExpenses.length ? 'Desmarcar Todas' : 'Selecionar Todas'}
              </button>
              ...
              if (window.confirm(`Tem certeza que deseja excluir ${selectedExpenses.length} despesa${selectedExpenses.length !== 1 ? 's' : ''}?`)) {
                try {
                  const {
                    error
                  } = await supabase.from('expenses').update({
            ...
            }} className="flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-dark-text-primary bg-gradient-to-r from-red-600 to-pink-600 hover:from-red-700 hover:to-pink-700 rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl">
          ...
          <table className="w-full">
            <thead className="bg-gradient-light dark:from-gray-800 dark:to-gray-750 border-b-2 border-light-border dark:border-dark-border">
              <tr>
                <th className="px-6 py-4 text-left">
                  <input type="checkbox" checked={selectedExpenses.length === filteredExpenses.length && filteredExpenses.length > 0} onChange={() => {
                  if (selectedExpenses.length === filteredExpenses.length) {
                    setSelectedExpenses([]);
                  } else {
                    setSelectedExpenses(filteredExpenses.map(e => e.id));
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {loading ? <tr>
                ...
                </tr> : filteredExpenses.map(expense => <tr key={expense.id} className={`group transition-all duration-200 ${selectedExpenses.includes(expense.id) ? 'bg-blue-50 dark:bg-blue-900/20' : 'hover:bg-gradient-to-r hover:from-gray-50/50 hover:to-gray-100/50 dark:hover:from-gray-800/50 dark:hover:to-gray-750/50'}`}>
                    <td className="px-6 py-4">
                      <input type="checkbox" checked={selectedExpenses.includes(expense.id)} onChange={e => {
                  if (e.target.checked) {
                    ...
                    <td className="px-6 py-4">
                      {expense.paid_value ? <>
                          ...
                          {expense.payment_date && <p className="text-xs text-theme-secondary">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center gap-2">
                        <button onClick={() => handleViewDetails(expense)} className="p-2 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/30 rounded-lg transition-all duration-200" title="Ver detalhes">
                          <Eye className="w-4 h-4" />
                        </button>

                        {/* Botão Dar Baixa - Só aparece se status != Paid */}
                        {expense.status !== 'Paid' && <button onClick={() => handleDarBaixa(expense)} className="p-2 text-green-600 dark:text-green-400 hover:bg-green-100 dark:hover:bg-green-900/30 rounded-lg transition-all duration-200" title="Dar Baixa">
                            <CheckCircle className="w-4 h-4" />
                          </button>}
      ...
      {/* Modais */}
      {isDetailsModalOpen && selectedExpenseForAction && <ExpenseDetailsModal isOpen={isDetailsModalOpen} onClose={() => {
      setIsDetailsModalOpen(false);
      setSelectedExpenseForAction(null);
    }} expense={selectedExpenseForAction} />}

      {isEditModalOpen && <NovaDespesaModal isOpen={isEditModalOpen} onClose={() => {
      setIsEditModalOpen(false);
      setSelectedExpenseForAction(null);
    }} initialData={selectedExpenseForAction} isEditing={!!selectedExpenseForAction} unidadeId={globalFilters.unitId} onSave={() => {
      fetchExpenses();
      setIsEditModalOpen(false);
      setSelectedExpenseForAction(null);
    }} />}

      {isDeleteModalOpen && selectedExpenseForAction && <DeleteConfirmationModal expense={selectedExpenseForAction} isOpen={isDeleteModalOpen} onClose={() => {
      ...
      {isPaymentDateModalOpen && selectedExpenseForAction && <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
