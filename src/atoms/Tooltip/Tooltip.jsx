...
export function Tooltip({
  ...
  return <div className={`relative inline-block ${className}`} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      {children}

      <AnimatePresence>
        {isVisible && <motion.div variants={tooltipVariants} initial="initial" animate="animate" exit="exit" className={`
...
export function KeyboardShortcut({
  ...
  return <Tooltip content={<div className="flex items-center gap-2">
          <span>{description}</span>
          <div className="flex items-center gap-1">
            {keys.map((key, index) => <React.Fragment key={key}>
                <kbd className="px-2 py-1 text-xs bg-gray-600 dark:bg-gray-300 dark:bg-gray-600 dark:text-theme-primary rounded border">
                  {key}
                </kbd>
                {index < keys.length - 1 && <span className="text-light-text-muted dark:text-dark-text-muted">+</span>}
              </React.Fragment>)}
          </div>
...
export function InfoTooltip({
  ...
  return <Tooltip content={<div className="max-w-xs">
