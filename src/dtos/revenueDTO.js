...
export class CreateRevenueDTO {
  ...
  constructor(data) {
     ...
     * @type {string|null}
     */
    this.category_id = safeData.category_id || null;
     ...
     * @type {string|null}
     */
    this.expected_receipt_date =
      safeData.expected_receipt_date ||
      this._calculateSettlementDate(safeData.payment_method);
  ...
  validate() {
    ...
    if (
      this.accrual_start_date &&
      !this._isValidDate(this.accrual_start_date)
    ) {
      errors.push(
        'Campo "accrual_start_date" deve estar no formato YYYY-MM-DD'
      );
    }
    ...
    if (
      this.expected_receipt_date &&
      !this._isValidDate(this.expected_receipt_date)
    ) {
      errors.push(
        'Campo "expected_receipt_date" deve estar no formato YYYY-MM-DD'
      );
    }

    if (
      this.actual_receipt_date &&
      !this._isValidDate(this.actual_receipt_date)
    ) {
      errors.push(
        'Campo "actual_receipt_date" deve estar no formato YYYY-MM-DD'
      );
    }
  ...
  toDatabase() {
    ...
    // eslint-disable-next-line no-console
    console.log(
      '📦 DTO.toDatabase() - Campos aprovados:',
...
export class RevenueResponseDTO {
  ...
  toPlainObject() {
    ...
    const baseFields = {
      ...
      party_id: this.party_id, // ✅ FIX: Adicionar party_id
    ...
    if (
      this.actual_receipt_date !== undefined &&
      this.actual_receipt_date !== null
    ) {
  ...
  toDisplay() {
    return {
      ...
      valorBruto: this.gross_amount
        ? this._formatCurrency(this.gross_amount)
        : null,
      valorLiquido: this.net_amount
        ? this._formatCurrency(this.net_amount)
        : null,
