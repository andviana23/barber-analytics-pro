...
const ImportExpensesFromOFXModal = ({
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4 dark:bg-opacity-70">
      <div className="card-theme flex max-h-[85vh] w-full max-w-4xl flex-col overflow-hidden rounded-lg shadow-xl dark:bg-dark-surface">
        ...
        <div className="flex-shrink-0 border-b border-light-border bg-light-bg px-6 py-2.5 dark:border-dark-border dark:bg-dark-bg dark:bg-dark-surface">
          <div className="flex items-center justify-center gap-8">
            ...
            title
                ...
                <div className={`flex h-5 w-5 items-center justify-center rounded-full border-2 ${currentStep >= step ? 'border-purple-600 bg-purple-600 text-white dark:border-purple-400 dark:bg-purple-400' : 'border-gray-300 dark:border-gray-600'}`}>
                  {currentStep > step ? <CheckCircle className="h-3 w-3" /> : <span className="text-xs font-medium">{step}</span>}
                </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {/* STEP 1: Upload OFX */}
          {currentStep === 1 && <div className="space-y-4 p-6">
              {/* Info da Unidade Selecionada */}
              {selectedUnit && <div className="flex items-center gap-2 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 dark:border-blue-800 dark:bg-blue-900/20">
              ...
              <div>
                ...
                <select value={selectedAccount} onChange={e => setSelectedAccount(e.target.value)} className="card-theme text-theme-primary dark:text-dark-text-primary w-full rounded-lg border border-light-border px-3 py-2 focus:border-purple-500 focus:ring-2 focus:ring-purple-500 dark:border-dark-border dark:bg-gray-700" disabled={!effectiveUnitId}>
                  <option value="">
              ...
              <div>
                ...
                <div onClick={() => fileInputRef.current?.click()} className="cursor-pointer rounded-lg border-2 border-dashed border-light-border bg-light-bg p-6 text-center transition-colors hover:border-purple-400 dark:border-dark-border dark:bg-dark-bg dark:bg-dark-surface dark:hover:border-purple-500">
              ...
              {!selectedAccount && effectiveUnitId && <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-3 dark:border-yellow-800 dark:bg-yellow-900/20">
                  <p className="text-xs text-yellow-800 dark:text-yellow-300">
                    Selecione uma conta bancária para começar
                  </p>
                </div>}

              {!effectiveUnitId && <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-3 dark:border-yellow-800 dark:bg-yellow-900/20">
                  <p className="text-xs text-yellow-800 dark:text-yellow-300">
          ...
          {/* STEP 2: Seleção de Categorias */}
          {currentStep === 2 && normalizedData.length > 0 && <div className="space-y-4 p-6">
              ...
              {categoriesLoading && <div className="py-4 text-center">
              ...
              {categoriesError && <div className="rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-800 dark:bg-red-900/20">
          ...
          {/* STEP 3: Preview e Confirmação */}
          {currentStep === 3 && enrichedData.length > 0 && previewStats && <div className="space-y-4 p-6">
              {/* Resumo - Compacto */}
              ...
              <div>
                <h4 className="mb-2 text-xs font-semibold text-gray-700 dark:text-gray-300 dark:text-gray-600 dark:text-theme-secondary">
                ...
                {enrichedData.length > 10 && <p className="text-theme-secondary dark:text-light-text-muted dark:text-dark-text-muted mt-2 text-center text-xs">
        ...
        {/* Validation Errors */}
        {validationErrors.length > 0 && <div className="border-t border-red-200 bg-red-50 px-6 py-3 dark:border-red-800 dark:bg-red-900/20">
        ...
        <div className="flex flex-shrink-0 items-center justify-between border-t border-light-border bg-light-bg px-6 py-4 dark:border-dark-border dark:bg-dark-bg dark:bg-dark-surface">
          <div className="text-theme-secondary dark:text-light-text-muted dark:text-dark-text-muted flex items-center text-xs">
            {file && <span className="max-w-xs truncate">
                {file.name} ({(file.size / 1024).toFixed(1)} KB)
              </span>}
          </div>

          <div className="flex items-center gap-2">
            {currentStep > 1 && <button type="button" onClick={() => setCurrentStep(currentStep - 1)} className="rounded-lg border border-light-border px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300 dark:text-gray-600 transition-colors hover:bg-light-bg dark:border-dark-border dark:bg-dark-bg dark:text-theme-secondary dark:hover:bg-gray-700">
                Voltar
              </button>}

            <button type="button" onClick={handleClose} className="rounded-lg border border-light-border px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300 dark:text-gray-600 transition-colors hover:bg-light-bg dark:border-dark-border dark:bg-dark-bg dark:text-theme-secondary dark:hover:bg-gray-700">
              Cancelar
            </button>

            {currentStep === 2 && <button type="button" onClick={handleProceedToPreview} className="text-dark-text-primary flex items-center gap-2 rounded-lg bg-purple-600 px-4 py-1.5 text-sm transition-colors hover:bg-purple-700 dark:bg-purple-500 dark:hover:bg-purple-600">
                Gerar Preview
                <ArrowRight className="h-3.5 w-3.5" />
              </button>}

            {currentStep === 3 && <button type="button" onClick={handleConfirmImport} disabled={isSubmitting} className="text-dark-text-primary flex items-center gap-2 rounded-lg bg-green-600 px-4 py-1.5 text-sm transition-colors hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-green-500 dark:hover:bg-green-600">
                {isSubmitting ? <>
