  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div className="card-theme max-h-[90vh] w-full max-w-6xl overflow-hidden rounded-lg shadow-xl">
        ...
        <div className="flex items-center justify-between border-b border-light-border p-6 dark:border-dark-border">
          ...
          <button onClick={handleClose} className="text-light-text-muted dark:text-dark-text-muted hover:text-theme-secondary hover:card-theme flex h-8 w-8 items-center justify-center rounded-lg transition-colors">
            <X className="h-5 w-5" />
          </button>
        </div>
          ...
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center space-x-6">
              ...
              title
                  ...
                  <div className={`flex h-6 w-6 items-center justify-center rounded-full border-2 ${currentStep >= step ? 'border-blue-600 bg-blue-600 text-white' : 'border-gray-300'}`}>
                    {currentStep > step ? <CheckCircle className="h-4 w-4" /> : <span className="text-xs font-medium">
                        {step === 2.5 ? '★' : step}
                      </span>}
                  </div>
        ...
        <div className="max-h-[60vh] flex-1 overflow-y-auto">
          {currentStep === 1 && <div className="space-y-6 p-6">
              {/* Seleção de Conta */}
              <div>
                ...
                <select value={selectedAccount} onChange={e => setSelectedAccount(e.target.value)} className="w-full rounded-lg border border-light-border px-3 py-2 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 dark:border-dark-border">
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                ...
                skipFirstRow: e.target.checked
                  ...
                  <label htmlFor="skipFirstRow" className="ml-2 text-sm text-gray-700 dark:text-gray-300 dark:text-gray-600 dark:text-theme-secondary">
              ...
              <div>
                ...
                <div onClick={() => fileInputRef.current?.click()} className="cursor-pointer rounded-lg border-2 border-dashed border-light-border p-8 text-center transition-colors hover:border-gray-400 dark:border-dark-border">
          ...
          {currentStep === 2 && parseResult && <div className="space-y-6 p-6">
              ...
              <div>
                ...
                <div className="space-y-4">
                  {/* Required Columns */}
                  <div>
                    <h4 className="mb-3 text-sm font-medium text-gray-700 dark:text-gray-300 dark:text-gray-600 dark:text-theme-secondary">
                      Campos Obrigatórios
                    </h4>
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                      {requiredColumns.map(column => <div key={column.key}>
                          <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300 dark:text-gray-600 dark:text-theme-secondary">
                            {column.label} *
                          </label>
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full rounded-lg border border-light-border dark:border-dark-border">
                      <thead className="bg-light-bg dark:bg-dark-bg">
                        <tr>
                          {parseResult.headers.map(header => <th key={header} className="text-theme-secondary border-b border-light-border px-3 py-2 text-left text-xs font-medium uppercase tracking-wider dark:border-dark-border">
                              {header}
                            </th>)}
                        </tr>
                      </thead>
                      <tbody className="card-theme divide-y divide-gray-200">
                        {parseResult.sample.map((row, index) => <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                            {parseResult.headers.map(header => <td key={header} className="text-theme-primary border-b border-light-border px-3 py-2 text-sm dark:border-dark-border">
                                {row[header] || '-'}
                              </td>)}
                          </tr>)}
                      </tbody>
          ...
          {currentStep === 3 && previewData.length > 0 && <div className="space-y-6 p-6">
              {/* Summary */}
              <div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
                  ...
                  <table className="min-w-full rounded-lg border border-light-border dark:border-dark-border">
                    ...
                    <tbody className="card-theme divide-y divide-gray-200">
                      {previewData.map((row, index) => <tr key={index} className={row.hasErrors ? 'bg-red-50' : 'bg-white'}>
                          ...
                          <td className="text-theme-primary px-3 py-2 text-sm">
                            <span className={`rounded-full px-2 py-1 text-xs ${row.tipo === 'C' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
        ...
        {/* Validation Errors */}
        {validationErrors.length > 0 && <div className="border-t border-red-200 bg-red-50 px-6 py-3">
        ...
        <div className="flex items-center justify-between border-t border-light-border bg-light-bg p-6 dark:border-dark-border dark:bg-dark-bg">
          ...
          <div className="flex items-center gap-3">
          ...
          }} className="rounded-lg border border-light-border px-4 py-2 text-gray-700 dark:text-gray-300 dark:text-gray-600 transition-colors hover:bg-light-bg dark:border-dark-border dark:bg-dark-bg dark:text-theme-secondary" data-testid="btn-back">
                Voltar
              </button>}

            <button type="button" onClick={handleClose} className="rounded-lg border border-light-border px-4 py-2 text-gray-700 dark:text-gray-300 dark:text-gray-600 transition-colors hover:bg-light-bg dark:border-dark-border dark:bg-dark-bg dark:text-theme-secondary">
              Cancelar
            </button>

            {currentStep === 1 && parseResult && <button type="button" onClick={() => setCurrentStep(2)} className="text-dark-text-primary flex items-center gap-2 rounded-lg bg-blue-600 px-6 py-2 transition-colors hover:bg-blue-700">
                Continuar
            ...
            {currentStep === 2 && <>
                ...
                <label className="mr-2 flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 dark:text-gray-600 dark:text-theme-secondary">
                  <input type="checkbox" checked={enableAutoMatch} onChange={e => setEnableAutoMatch(e.target.checked)} className="rounded border-light-border text-blue-600 focus:ring-blue-500 dark:border-dark-border" />
                  <span>Executar auto-match de reconciliação</span>
                </label>

                <button type="button" onClick={handleProceedToPreview} className="text-dark-text-primary flex items-center gap-2 rounded-lg bg-blue-600 px-6 py-2 transition-colors hover:bg-blue-700" data-testid="btn-proceed-from-mapping">
                  {enableAutoMatch ? 'Executar Auto-Match' : 'Gerar Preview'}
                  <ArrowRight className="h-4 w-4" />
                </button>
              </>}

            {currentStep === 3 && <button type="button" onClick={handleExecuteImport} disabled={isProcessing || previewData.filter(row => !row.hasErrors).length === 0} className="text-dark-text-primary flex items-center gap-2 rounded-lg bg-green-600 px-6 py-2 transition-colors hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-50">
                {isProcessing ? <>
