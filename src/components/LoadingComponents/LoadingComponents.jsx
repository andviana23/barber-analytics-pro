...
export function LoadingSpinner({
  ...
  const spinner = <div className="flex flex-col items-center justify-center space-y-3">
      ...
      <div className="text-sm text-theme-secondary dark:text-gray-300 dark:text-gray-600 font-medium" role="status" aria-live="polite">
        {text}
      </div>
  ...
  if (fullScreen) {
    return <motion.div initial={{
      ...
      opacity: 0
        ...
        `} role="dialog" aria-modal="true" aria-labelledby="loading-title">
...
export function SkeletonLoader({
  ...
  return <div className={`animate-pulse space-y-4 ${className}`} aria-hidden="true">
      ...
      <div className="space-y-2">
        {Array.from({
        length: lines
      }).map((_, index) => <div key={index} className={`
              h-4 bg-gray-200 dark:bg-gray-700 rounded
              ${index === lines - 1 ? 'w-3/4' : 'w-full'}
            `} />)}
      </div>

      {showButton && <div className="h-10 bg-gray-200 dark:bg-gray-700 rounded w-32" />}
...
export function CardSkeleton({
  ...
  return <div className={`bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 ${className}`}>
...
export function TableSkeleton({
  ...
  return <div className={`bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden ${className}`}>
      {/* Header */}
      <div className="border-b border-light-border dark:border-dark-border p-4">
        ...
        gridTemplateColumns: `repeat(${columns}, 1fr)`
          ...
          {Array.from({
          length: columns
        }).map((_, index) => <div key={index} className="h-4 bg-gray-200 dark:bg-gray-700 rounded" />)}
        </div>
      </div>
      ...
      <div className="divide-y divide-gray-200 dark:divide-gray-700">
        ...
        length: rows
          ...
          gridTemplateColumns: `repeat(${columns}, 1fr)`
              ...
              {Array.from({
            length: columns
          }).map((_, colIndex) => <div key={colIndex} className={`
...
export function ProgressBar({
  ...
  return <div className={`w-full ${className}`}>
      <div className="flex justify-between items-center mb-2">
        ...
        {showPercentage && <span className="text-sm text-theme-secondary dark:text-light-text-muted dark:text-dark-text-muted">
            {Math.round(percentage)}%
          </span>}
      </div>

      <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5" role="progressbar" aria-valuenow={progress} aria-valuemin={0} aria-valuemax={max} aria-label={`${label}: ${Math.round(percentage)}% concluído`}>
        <motion.div className="bg-blue-600 h-2.5 rounded-full transition-all duration-300" initial={{
        width: 0
      }} animate={{
        width: `${percentage}%`
      }} transition={{
...
export function FormLoadingOverlay({
  ...
  return <motion.div initial={{
    ...
    opacity: 0
  }} className="absolute inset-0 card-theme/80 dark:bg-dark-surface/80 backdrop-blur-sm z-10 flex items-center justify-center" role="status" aria-live="polite">
