...
export function ContrastTester() {
  ...
  const getStatusColor = (level) => {
    switch (level) {
      ...
      case 'AA': return 'text-yellow-600 dark:text-yellow-400';
          ...
          >
            <Card className="p-4 space-y-3">
              ...
              <div className="flex justify-between items-center text-sm">
                <div>
                  <span className="text-gray-600 dark:text-gray-300">Contraste:</span>
                  <span className={`ml-1 font-mono ${getStatusColor(result.level)}`}>
                    {result.contrast}:1
                  </span>
                </div>
                <div className={`px-2 py-1 rounded text-xs font-medium ${getStatusColor(result.level)} bg-opacity-10`}>
                  {result.level}
                </div>
              </div>
              ...
              <div className="flex items-center space-x-2 text-xs">
                <div className="flex items-center space-x-1">
                  ...
                  />
                  <span className="text-gray-500 font-mono">{result.foreground}</span>
                </div>
                ...
                <div className="flex items-center space-x-1">
                  ...
                  />
                  <span className="text-gray-500 font-mono">{result.background}</span>
                </div>
              </div>
            </Card>
          </motion.div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-blue-700 dark:text-blue-200 mt-1">
              ...
              {testResults.filter(r => r.level === 'Falha').length} precisam melhorar
            </p>
          </div>
...
export function ThemeValidator() {
  ...
  const tabs = [
    ...
    { id: 'responsiveness', label: 'Responsividade', icon: '📱' }
  ];
      ...
      >
        ...
        {activeTab === 'accessibility' && (
          <div className="space-y-4">
            <Card className="p-6">
              <h3 className="text-lg font-semibold mb-4">Checklist de Acessibilidade</h3>
        ...
        {activeTab === 'responsiveness' && (
          <div className="space-y-4">
            <Card className="p-6">
              <h3 className="text-lg font-semibold mb-4">Breakpoints Suportados</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  { size: 'xs', width: '375px+', label: 'Mobile S' },
                  { size: 'sm', width: '640px+', label: 'Mobile L' },
                  { size: 'md', width: '768px+', label: 'Tablet' },
                  { size: 'lg', width: '1024px+', label: 'Desktop' },
                  { size: 'xl', width: '1280px+', label: 'Desktop L' },
                  { size: '2xl', width: '1536px+', label: 'Desktop XL' },
                  { size: '3xl', width: '1920px+', label: '4K' }
                ].map(bp => (
                  <div key={bp.size} className="text-center p-3 bg-gray-50 dark:bg-gray-800 rounded">
                    <div className="font-mono text-sm font-medium">{bp.size}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">{bp.width}</div>
                    <div className="text-xs text-gray-500 mt-1">{bp.label}</div>
                  </div>
                ))}
              </div>
            </Card>
