...
import { Target, TrendingUp, DollarSign, Users, Package, CreditCard, ArrowRight, Zap, Award, AlertTriangle, Clock, ChevronRight, Sparkles } from 'lucide-react';
...
export const MetasCard = ({
  ...
  if (loading) {
    return <div className="card-theme rounded-3xl p-8 shadow-2xl">
        ...
        <div className="space-y-4">
          {[...Array(3)].map((_, i) => <div key={i} className="h-36 animate-pulse rounded-2xl bg-gradient-light dark:from-gray-700/50 dark:to-gray-600/50"></div>)}
        </div>
      </div>;
  }
  if (!goals || goals.length === 0) {
    return <div className="card-theme rounded-3xl p-8 shadow-2xl">
        <div className="mb-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              {unitName && <p className="text-theme-secondary dark:text-light-text-muted dark:text-dark-text-muted mt-1 text-sm font-medium">
                  {unitName}
                </p>}
            </div>
          </div>
        ...
        <div className="py-12 text-center">
          <div className="mb-6 inline-flex h-20 w-20 items-center justify-center rounded-3xl card-theme shadow-lg dark:bg-gray-700">
          ...
          <button onClick={() => navigate('/cadastros/metas')} className="text-dark-text-primary inline-flex items-center gap-2 rounded-xl bg-gradient-primary px-6 py-3 font-semibold shadow-lg shadow-blue-500/30 transition-all duration-300 hover:scale-105 hover:from-blue-700 hover:to-blue-800 hover:shadow-xl hover:shadow-blue-500/40">
  ...
  return <div className="card-theme hover:shadow-3xl rounded-3xl p-8 shadow-2xl transition-shadow duration-300">
      {/* Header */}
      <div className="mb-8 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="relative">
        ...
        <button onClick={() => navigate('/cadastros/metas')} className="text-dark-text-primary group flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-2.5 font-semibold shadow-lg shadow-blue-500/30 transition-all duration-300 hover:scale-105 hover:bg-blue-700 hover:shadow-xl hover:shadow-blue-500/40">
          Ver todas
      ...
      <div className="space-y-5">
        ...
        return <div key={goal.id} onClick={() => navigate('/cadastros/metas')} className={`group relative overflow-hidden rounded-2xl border-2 ${isAchieved ? 'border-green-300 bg-gradient-to-br from-green-50 to-emerald-50 dark:border-green-700 dark:from-green-900/30 dark:to-emerald-900/20' : 'border-gray-200 bg-gradient-to-br from-gray-50 to-white dark:border-gray-700 dark:from-gray-800/50 dark:to-gray-900/50'} cursor-pointer p-6 transition-all duration-300 hover:scale-[1.02] hover:shadow-xl ${statusInfo.glowColor}`}>
              {/* Background Pattern */}
              ...
              <div className="relative">
                ...
                <div className="mb-4 flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`rounded-xl p-3 ${typeInfo.iconBg} shadow-md transition-transform duration-300 group-hover:scale-110`}>
                  ...
                  {/* Status Badge */}
                  <div className={`flex items-center gap-2 rounded-full px-4 py-2 ${statusInfo.badgeBg} text-white shadow-lg ${statusInfo.glowColor}`}>
                    <StatusIcon className="h-4 w-4" />
                    <span className="text-sm font-bold">
                      {formatPercent(percentage)}
                    </span>
                    {isAchieved && <Sparkles className="h-4 w-4 animate-pulse" />}
                  </div>
                </div>
                ...
                <div className="mb-4">
                  <div className="relative h-3 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700 shadow-inner">
                    <div className={`absolute left-0 top-0 h-full bg-gradient-to-r ${statusInfo.progressGradient} rounded-full shadow-lg transition-all duration-1000 ease-out`} style={{
                ...
                <div className="flex items-center justify-between">
                  ...
                  {goal.remaining_value > 0 && !isAchieved && <div className="card-theme flex items-center gap-2 rounded-lg px-3 py-1.5 dark:bg-gray-700/50">
                  ...
                  {isAchieved && <div className="flex items-center gap-2 rounded-lg bg-green-100 px-3 py-1.5 dark:bg-green-900/40">
      ...
      {/* Footer */}
      {goals.length > 5 && <button onClick={() => navigate('/cadastros/metas')} className="mt-6 w-full rounded-xl border-2 border-dashed border-blue-300 bg-blue-50 py-3 text-sm font-bold text-blue-600 transition-all duration-300 hover:bg-blue-100 hover:text-blue-700 dark:border-blue-700 dark:bg-blue-900/20 dark:text-blue-400 dark:hover:bg-blue-900/30 dark:hover:text-blue-300">
