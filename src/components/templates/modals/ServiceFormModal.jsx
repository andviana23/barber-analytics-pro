      ...
      <div className="card-theme w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            {loadingCategories ? <div className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-light-border dark:border-dark-border bg-light-surface dark:bg-dark-hover">
                <div className="w-4 h-4 border-2 border-primary border-t-transparent rounded-full animate-spin" />
                <span className="text-sm text-theme-secondary">
                  Carregando categorias...
                </span>
              </div> : categories.length > 0 ? <>
                <select value={formData.categoryId} onChange={e => handleChange('categoryId', e.target.value)} disabled={loading} className="w-full px-4 py-2.5 rounded-lg border card-theme dark:bg-dark-surface text-theme-primary focus:outline-none focus:ring-2 focus:ring-primary/20 disabled:opacity-60 disabled:cursor-not-allowed border-light-border dark:border-dark-border focus:border-primary" required>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              {formData.price && <p className="text-xs text-theme-muted mt-1">
          ...
          {/* Cálculo da Comissão */}
          {formData.price && formData.commissionPercentage && <div className="p-4 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg">
          ...
          {isEditing && <div className="flex items-center justify-between p-4 bg-light-bg dark:bg-dark-bg dark:bg-dark-surface/50 rounded-lg border border-theme-border">
              <div>
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" checked={formData.active} onChange={e => handleChange('active', e.target.checked)} disabled={loading} className="sr-only peer" />
