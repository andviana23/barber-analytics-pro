...
const CategoryHierarchicalDropdown = ({
  ...
  return <div className="w-full">
      {/* Label */}
      {label && <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 dark:text-gray-600 mb-2">
          {label}
      ...
      <div className="relative">
        ...
        {/* Ícone de Dropdown */}
        {showIcon && <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
            <ChevronDown className={`w-5 h-5 ${error ? 'text-red-400 dark:text-red-500' : 'text-gray-400 dark:text-gray-500'}`} />
          </div>}
      </div>

      {/* Helper Text */}
      {helperText && !error && <p id="helper-text" className="mt-1 text-sm text-theme-secondary dark:text-light-text-muted dark:text-dark-text-muted">
          {helperText}
        </p>}
