...
const CreateProductModal = ({ isOpen, onClose, onCreate, loading = false }) => {
  ...
  const handleSubmit = async e => {
    ...
    // Converter valores numéricos (manter camelCase para o service)
    const dataToSubmit = {
      unit_id: selectedUnit?.id, // Obrigatório: ID da unidade
      name: formData.name.trim(),
      description: formData.description?.trim() || null,
      category_id: formData.category_id || null, // UUID da categoria
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div className="card-theme max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-lg shadow-xl dark:bg-dark-surface">
        ...
        <div className="flex items-center justify-between border-b border-light-border p-6 dark:border-dark-border">
          ...
          <button
            onClick={handleClose}
            className="text-light-text-muted dark:text-dark-text-muted hover:text-theme-secondary hover:card-theme dark:text-theme-secondary flex h-8 w-8 items-center justify-center rounded-lg transition-colors dark:text-gray-600 dark:hover:bg-gray-700 dark:hover:text-gray-300"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <div className="relative">
                  <Tag className="text-theme-muted absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 transform" />
                  <select
                    name="category_id"
                    value={formData.category_id}
                    onChange={handleChange}
                    disabled={loadingCategories}
                    className="input-theme w-full py-2 pl-10 pr-3"
                  >
          ...
          <div className="flex items-center justify-end gap-3 border-t border-light-border pt-4 dark:border-dark-border">
            <button
              type="button"
              onClick={handleClose}
              className="dark:text-theme-secondary rounded-lg border border-light-border px-4 py-2 text-gray-700 transition-colors hover:bg-light-bg dark:border-dark-border dark:bg-dark-bg dark:text-gray-300 dark:text-gray-600 dark:hover:bg-gray-700"
              disabled={loading}
            >
              Cancelar
            </button>
