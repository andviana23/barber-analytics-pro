...
export default function CommissionsTable({ commissions, onSave, loading }) {
  ...
  const EditableInput = ({ item }) => (
    <div className="flex flex-col items-center gap-1">
      <input
        ...
        className={`input-theme w-20 text-right ${
          validationError
            ? 'border-feedback-light-error dark:border-feedback-dark-error focus:ring-feedback-light-error/50'
            : ''
        }`}
  ...
  const ActionButtons = ({ item }) => {
    if (editId === item.serviceId) {
      return (
        <div className="flex items-center justify-center gap-2">
          <button
            className="p-1.5 rounded-lg text-feedback-light-success dark:text-feedback-dark-success hover:bg-light-bg dark:hover:bg-dark-hover transition-colors disabled:opacity-50"
          ...
          <button
            className="p-1.5 rounded-lg text-feedback-light-error dark:text-feedback-dark-error hover:bg-light-bg dark:hover:bg-dark-hover transition-colors"
    ...
    return (
      <button
        className="p-1.5 rounded-lg text-primary hover:bg-light-bg dark:hover:bg-dark-hover transition-colors"
        onClick={() =>
  ...
  return (
    <div className="w-full space-y-3">
      ...
      <div className="hidden md:block overflow-x-auto rounded-lg border border-light-border dark:border-dark-border shadow-sm">
        <table className="w-full text-sm bg-light-surface dark:bg-dark-surface">
          <thead>
      ...
      <div className="md:hidden space-y-3">
        {commissions.map(item => (
          <div
            key={item.serviceId}
            className="bg-light-surface dark:bg-dark-surface border border-light-border dark:border-dark-border rounded-lg p-4 shadow-sm"
          >
            {/* Nome do Serviço */}
