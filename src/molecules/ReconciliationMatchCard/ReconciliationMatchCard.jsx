...
const ReconciliationMatchCard = ({
  ...
  const getMatchStatus = () => {
    switch (status) {
      case 'Confirmado': return { color: 'green', label: 'Confirmado' };
      case 'Rejeitado': return { color: 'red', label: 'Rejeitado' };
      case 'Ajustado': return { color: 'blue', label: 'Ajustado' };
      default: return { color: 'gray', label: 'Pendente' };
    }
  };
  ...
  const cardClasses = `
    ...
    ${compact ? 'p-3' : 'p-4'}
    ${confidence.level === 'high' ? 'border-green-200' : 
      confidence.level === 'medium' ? 'border-yellow-200' :
      confidence.level === 'low' ? 'border-orange-200' : 'border-red-200'}
    ${className}
  `;
  ...
  const confidenceBarClasses = `
    ...
    ${confidence.color === 'green' ? 'bg-green-500' :
      ...
      confidence.color === 'orange' ? 'bg-orange-500' : 'bg-red-500'}
  `;
    ...
    <div className={cardClasses}>
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex items-center ml-2 flex-shrink-0 space-x-2">
          <StatusBadge 
            status={matchStatus.color === 'green' ? 'received' : 
                    matchStatus.color === 'red' ? 'cancelled' : 'pending'} 
            size={compact ? 'sm' : 'md'}
          />
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          <div className="flex items-center mb-2">
            ...
            <span className="text-sm font-medium text-blue-700">Transação Interna</span>
          </div>
          ...
          <div className="space-y-1">
            ...
            {internal_transaction?.description && (
              <div className="text-xs text-blue-600 truncate" title={internal_transaction.description}>
                {internal_transaction.description}
              </div>
            )}
            {internal_transaction?.party_name && (
              <div className="flex items-center text-xs text-blue-600">
                ...
                <span className="truncate">{internal_transaction.party_name}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
          <div className="flex items-center mb-2">
            ...
            <span className="text-sm font-medium text-gray-700">Extrato Bancário</span>
          </div>
          ...
          <div className="space-y-1">
            ...
            {bank_statement?.description && (
              <div className="text-xs text-gray-600 truncate" title={bank_statement.description}>
                {bank_statement.description}
              </div>
            )}
      ...
      {hasSignificantDifferences && (
        <div className="mb-3 p-2 bg-amber-50 border border-amber-200 rounded-lg">
          <div className="flex items-center mb-1">
            ...
            <span className="text-sm font-medium text-amber-700">Diferenças Identificadas</span>
          </div>
          ...
          <div className="space-y-1">
            {Math.abs(valueDifference) > 0.01 && (
              <div className="flex items-center text-xs text-amber-600">
                ...
                Valor: {valueDifference > 0 ? '+' : ''}{formatCurrency(valueDifference)}
              </div>
            )}
            {Math.abs(dateDifference) > 0 && (
              <div className="flex items-center text-xs text-amber-600">
                <Calendar className="w-3 h-3 mr-1" />
                Data: {dateDifference > 0 ? '+' : ''}{dateDifference} dias
              </div>
            )}
          </div>
        </div>
      ...
      {isExpanded && (
        <div className="border-t border-gray-200 pt-3 mt-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <div>
              <h4 className="font-medium text-gray-900 mb-2">Transação Interna</h4>
            ...
            {/* Detalhes do Extrato */}
            <div>
              <h4 className="font-medium text-gray-900 mb-2">Extrato Bancário</h4>
              <div className="space-y-1 text-gray-600">
        ...
        <div className="flex items-center justify-end space-x-2 mt-3 pt-3 border-t border-gray-200">
          {onViewDetails && (
            <button
              ...
              className="px-3 py-1.5 text-sm text-gray-600 hover:text-gray-800 hover:bg-gray-100 rounded-md flex items-center"
            >
          ...
          {onConfirm && (
            <button
              ...
              className="px-3 py-1.5 text-sm bg-green-600 text-white hover:bg-green-700 rounded-md flex items-center"
            >
