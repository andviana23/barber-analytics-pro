  ...
  return <div className={cardClasses}>
      ...
      <div className="mb-3 flex items-start justify-between">
        <div className="flex min-w-0 flex-1 items-center">
          <Link className={`mr-2 h-5 w-5 flex-shrink-0 text-${confidence.color}-500`} />
        ...
        <div className="ml-2 flex flex-shrink-0 items-center space-x-2">
          ...
          {showActions && <button type="button" onClick={() => setIsExpanded(!isExpanded)} className="hover:card-theme rounded p-1">
      ...
      <div className="mb-3 grid grid-cols-2 gap-3">
        ...
        <div className="rounded-lg border border-blue-200 bg-blue-50 p-3">
          <div className="mb-2 flex items-center">
            {internal_transaction?.type === 'receita' ? <TrendingUp className="mr-1 h-4 w-4 text-blue-500" /> : <TrendingDown className="mr-1 h-4 w-4 text-blue-500" />}
          ...
          <div className="space-y-1">
            ...
            {internal_transaction?.description && <div className="truncate text-xs text-blue-600" title={internal_transaction.description}>
                {internal_transaction.description}
              </div>}
            {internal_transaction?.party_name && <div className="flex items-center text-xs text-blue-600">
                {internal_transaction.party_type === 'cliente' ? <User className="mr-1 h-3 w-3" /> : <Building className="mr-1 h-3 w-3" />}
        ...
        <div className="rounded-lg border border-light-border bg-light-bg p-3 dark:border-dark-border dark:bg-dark-bg">
          ...
          <div className="space-y-1">
            ...
            {bank_statement?.description && <div className="text-theme-secondary truncate text-xs" title={bank_statement.description}>
                {bank_statement.description}
              </div>}
            {bank_statement?.balance_after && <div className="text-theme-secondary text-xs">
      ...
      {hasSignificantDifferences && <div className="mb-3 rounded-lg border border-amber-200 bg-amber-50 p-2">
      ...
      {/* Detalhes Expandidos */}
      {isExpanded && <div className="mt-3 border-t border-light-border pt-3 dark:border-dark-border">
          ...
          {/* Algoritmo de Match */}
          {differences && Object.keys(differences).length > 0 && <div className="mt-4 rounded-lg bg-light-bg p-3 dark:bg-dark-bg">
      ...
      {/* Ações */}
      {showActions && status === 'Pendente' && <div className="mt-3 flex items-center justify-end space-x-2 border-t border-light-border pt-3 dark:border-dark-border">
          {onViewDetails && <button type="button" onClick={() => onViewDetails(match)} className="text-theme-secondary hover:text-theme-primary hover:card-theme flex items-center rounded-md px-3 py-1.5 text-sm">
              <Eye className="mr-1 h-4 w-4" />
              Detalhes
            </button>}

          {hasSignificantDifferences && onAdjust && <button type="button" onClick={() => onAdjust(match)} className="flex items-center rounded-md px-3 py-1.5 text-sm text-blue-600 hover:bg-blue-50 hover:text-blue-800">
              <Edit3 className="mr-1 h-4 w-4" />
              Ajustar
            </button>}

          {onReject && <button type="button" onClick={() => onReject(match)} className="flex items-center rounded-md px-3 py-1.5 text-sm text-red-600 hover:bg-red-50 hover:text-red-800">
              <X className="mr-1 h-4 w-4" />
              Rejeitar
            </button>}

          {onConfirm && <button type="button" onClick={() => onConfirm(match)} className="text-dark-text-primary flex items-center rounded-md bg-green-600 px-3 py-1.5 text-sm hover:bg-green-700">
...
export const ReconciliationMatchCardPreview = () => {
  ...
  return <div className="max-w-4xl space-y-4 p-4">
