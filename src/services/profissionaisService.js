...
export class ProfissionaisService {
  ...
  static async getProfissionais(filters = {}) {
    ...
    try {
      ...
      console.log(
        '✅ Query executada com sucesso. Registros encontrados:',
        data?.length || 0
      );
      return data || [];
    } catch (error) {
      ...
      if (
        ...
        throw new Error(
          'Sem permissão para acessar profissionais. Verifique seu login ou contate o administrador.'
        );
      }
  ...
  static async createProfissional(profissionalData) {
    try {
      ...
      const professionalFields = {
        ...
        is_active:
          profissionalData.is_active !== undefined
      ...
      // (quando o profissional tiver credenciais de login)
  ...
  static async toggleProfissionalStatus(id) {
    try {
      ...
      throw new Error(
        `Erro ao alterar status do profissional: ${error.message}`
      );
    }
  }
   ...
   */
  static async getProfissionalStats(
    profissionalId,
    ...
    try {
      ...
      const ticketMedio =
        totalAtendimentos > 0 ? faturamento / totalAtendimentos : 0;
      const tempoMedio =
        atendimentos?.length > 0
          ? atendimentos.reduce(
              (sum, item) => sum + (item.duracao_minutos || 0),
              0
            ) / atendimentos.length
          : 0;
      ...
      if (
        filaError &&
        filaError.code !== 'PGRST116' &&
        filaError.code !== 'PGRST506'
      ) {
      ...
      throw new Error(
        `Erro ao buscar estatísticas do profissional: ${error.message}`
      );
    }
  }
   ...
   */
  static async getRankingProfissionais(
    unitId = null,
    mes = new Date().getMonth() + 1,
    ano = new Date().getFullYear()
  ) {
    try {
      ...
      const ranking = await Promise.all(
        profissionais.map(async profissional => {
          const stats = await this.getProfissionalStats(
            profissional.id,
            mes,
            ano
          );
      ...
      throw new Error(
        `Erro ao buscar ranking de profissionais: ${error.message}`
      );
    }
  }
  ...
  static async createProfessionalComplete(data) {
    try {
      ...
      if (professionalError) {
        throw new Error(
          `Erro ao criar profissional: ${professionalError.message}`
        );
      }
