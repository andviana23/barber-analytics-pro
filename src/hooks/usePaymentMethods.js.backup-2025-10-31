...
export const usePaymentMethods = (unitId, options = {}) => {
  ...
  const fetchPaymentMethods = useCallback(async (forceRefresh = false) => {
    try {
      ...
      const {
        ...
        includeInactive: currentIncludeInactive,
        enableCache: currentEnableCache,
      } = paramsRef.current;
      ...
      const { data: paymentMethods, error: fetchError } =
        await paymentMethodsService.getPaymentMethods(
          currentUnitId,
      ...
      const { data: statsData } =
        await paymentMethodsService.getPaymentMethodsStats(currentUnitId);
  ...
  const createPaymentMethod = useCallback(
    async paymentMethodData => {
      try {
        ...
        const { data: newPaymentMethod, error: createError } =
  ...
  const updatePaymentMethod = useCallback(
    async (id, updates) => {
      try {
        ...
        const { data: updatedPaymentMethod, error: updateError } =
          await paymentMethodsService.updatePaymentMethod(id, updates);
  ...
  const deletePaymentMethod = useCallback(
    async id => {
      try {
        ...
        const { data: deletedPaymentMethod, error: deleteError } =
   ...
   */
  const activatePaymentMethod = useCallback(
    async id => {
      try {
        setError(null);

        const { data: activatedPaymentMethod, error: activateError } =
