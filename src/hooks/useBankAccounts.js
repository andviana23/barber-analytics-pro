...
export function useBankAccounts(initialFilters = {}) {
  ...
  const loadBankAccounts = useCallback(async () => {
    try {
      ...
      const data = await bankAccountsService.getBankAccounts(unitId, incluirInativas);
  ...
  const createBankAccount = useCallback(async (accountData) => {
    try {
      ...
      const newAccount = await bankAccountsService.createBankAccount(accountData);
  ...
  const updateBankAccount = useCallback(async (id, updateData) => {
    try {
      ...
      const updatedAccount = await bankAccountsService.updateBankAccount(id, updateData);
   ...
   */
  const checkAccountExists = useCallback(async (bank, agency, accountNumber, unitId, excludeId = null) => {
    try {
      return await bankAccountsService.checkAccountExists(bank, agency, accountNumber, unitId, excludeId);
    } catch (err) {
      setError(err.message);
      return false;
    }
