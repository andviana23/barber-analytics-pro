...
export const useBankStatements = (accountId, startDate, endDate) => {
  ...
  const fetchStatements = useCallback(async (showLoading = true) => {
    if (!accountId) {
      setState(prev => ({ 
        ...prev, 
        statements: [], 
    ...
    const allStatementsKey = getCacheKey('all', accountId, startDate, endDate);
    ...
    if (cachedAll && cachedUnreconciled && 
    ...
    try {
      ...
      const statementsPromise = shouldFetchRange
        ? bankStatementsService.getStatements({ accountId, startDate, endDate })
  ...
  const getStatementById = useCallback(async (statementId) => {
    try {
      ...
      const localStatement = cachedStatements.find(stmt => stmt.id === statementId);
      if (localStatement) {
        return { success: true, data: localStatement };
      }

      // Se não encontrar, buscar no servidor
      const { data, error } = await bankStatementsService.getStatementById(statementId);
  ...
  const importStatements = useCallback(async (payload = {}) => {
    ...
    if (!targetAccountId) {
      return { success: false, error: 'Account ID eh obrigatorio para importacao' };
    }
    ...
    if (!statements && Array.isArray(payload.preview_data)) {
      statements = payload.preview_data
        ...
        .map(row => {
          ...
          return {
            transaction_date: row.transaction_date || row.data || row.date || null,
            description: row.description || row.descricao || '',
            amount: amount ?? 0,
            type: row.type ||
              (row.tipo === 'C' ? 'Credit' : row.tipo === 'D' ? 'Debit' : null),
            document: row.documento || row.document || null,
            balance_after: sanitizeAmount(row.balance_after ?? row.saldo) ?? null
          };
        })
    ...
    try {
      ...
      const { data, error } = await bankStatementsService.importStatements(servicePayload);
